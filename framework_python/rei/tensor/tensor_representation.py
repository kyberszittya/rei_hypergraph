...
from rei.hypergraph.representation.abstract_representations import AbstractTensorRepresentation
...
class NumpyTensorRepresentation(AbstractTensorRepresentation):
    ...
    def diag_degree_tensor(self):
        coeff = np.apply_along_axis(
            np.diag, 1, 1.0/((np.sum(self.W, axis=1) == np.sum(self.W, axis=2)).astype(float) + 1))
        __DD = np.apply_along_axis(np.diag, 1, np.sum(self.W, axis=1))
        return __DD @ coeff

    def calc_normalized_laplacian(self):
        # As per Ouvrard
        return 1.0/(self.Ii.shape[0] - 1) * self.L/np.sqrt(self.deg + 1)
    ...
    def entropy(self):
        return np.sum(self.entropy_vector())

    def entropy_projected_vector(self):
        v = np.abs(np.linalg.eig(self.Lp/self.total_deg)[0])
        return -v*np.log2(v)

    def entropy_projected(self):
        return np.sum(self.entropy_projected_vector())
