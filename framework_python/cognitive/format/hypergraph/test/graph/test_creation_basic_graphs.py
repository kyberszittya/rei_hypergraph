...
from cognitive.format.hypergraph.foundations.hypergraph_operators import \
    ...
    create_hyperedge, retrieve_part_hypergraph_node
...
def create_dir_edge(sys: HypergraphNode, qualified_name: str, root_node: list[str], nodes: list[str]):
    e = create_hyperedge("e"+''.join(nodes), 0, sys,
                     [(qualified_name+x, EnumRelationDirection.OUTWARDS) for x in nodes])
    for r in root_node:
...
def create_multitree():
    ...
    # Create edges
    fano_pr = "test/fano_graph/"
    create_dir_edge(test_system, fano_pr, ["4"], ["0", "1", "3"])
    create_dir_edge(test_system, fano_pr, ["2"], ["1", "5", "6"])
    create_dir_edge(test_system, fano_pr, ["1"], ["2", "3", "5"])
    create_dir_edge(test_system, fano_pr, ["6"], ["1", "2", "4"])
    create_dir_edge(test_system, fano_pr, ["1"], ["3", "4", "6"])
    create_dir_edge(test_system, fano_pr, ["3"], ["0", "4", "5"])
    create_dir_edge(test_system, fano_pr, ["1"], ["0", "2", "6"])
    return taxon, test_system
...
def test_fano_graph_basic():
    ...
    print()
    for set in fano_graph._subsets.values():
