...
from cognitive.physics.kinematics.kinematic_link import KinematicGraph, KinematicJoint


def graph_viz_kinematicgraph(kg: KinematicGraph, dot):
    graph = dot
...
def create_graph_view(sys: HypergraphNode):
    ...
    for entity in filter(lambda x: CognitiveEntity, sys._subsets.values()):
        for kg in filter(lambda x: KinematicGraph, entity._subsets.values()):
            graph_viz_kinematicgraph(kg, dot)
        dot.render(filename=f"{entity.id_name}")